  ...
  while (1)
  {
    if ((key = read_keypad()) != 'N')
    {
      ...
      if (key >= '0' && key <= '9') // If a numeric key is pressed
      {
        ...
        if (i == 2) // If two values have already been read
        {
          number[k] = arr[0] * 10 + arr[1]; // Store the two-digit number in an array
          k++;
        }
      }
    ...
    while ((star_pressed) && (k == 2))
    {
      ...
      *outa = segments[2];    // Select the corresponding segment for ones digit
      *outa = 0x0F;
