//Initially display 0. When switch 1 is pressed start displaying 0 to 9continuously.
...
void setup()
{
  ...
  *dirk = 0x00;//set PORT K as i/p port .
  ...
  volatile long  j;
  int num = 0;
  *outf = display(num);//initially display 0

  while (1)
  {
    l1:if (read_switch(ink, 0))//check if switch 1 is pressed
    {
      for (j = 0; j < 12000; j++); //a delay for switch debouncing
      while (1)
      {
        num = (num + 1) % 10;//display 0 to 9 continuosly
        *outf = display(num);
        ...
        for (j = 0; j < 200000; j++);//delay for display
      }
    }
  }
  return 0;
